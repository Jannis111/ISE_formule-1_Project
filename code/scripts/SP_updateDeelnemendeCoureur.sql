...
CREATE OR ALTER PROCEDURE SP_updateDeelnemendeCoureur
	@coureurID INT,
...
BEGIN 
	...
	BEGIN TRY
		...
		BEGIN TRY
		UPDATE DEELNEMENDECOUREUR
		SET TEAMNAAM = @teamName, FINISHTIJD = @finishTime, PUNTEN = @points, VOLGNUMMER = @followNummer
		WHERE COUREURID = @coureurID AND STARTDATUM = @startDate AND CIRCUITNAAM = @circuitName
	...
	END TRY
	BEGIN CATCH
		RAISERROR ('Something went wrong with the updateDeelnemendeCoureur procedure.' , 16, 1);
	END CATCH;
